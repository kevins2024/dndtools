...
**5e-ruleset files live under `rules/5e/` and `data/5e/` (2026-09-17), separate from generic engine plumbing.** Everything New Character/Level Up actually depend on — `classFeatures.js`, `subclasses.js`, `levelUp.js`, `diffLevelUp.js`, `spellcasting.js`, `species.js`, and 13 more, plus their backing `data/5e/classes/*.json`/`data/5e/subclasses/*.json`/etc. — is namespaced this way so a future ruleset (the Godot port, or "modernizing" the game itself) can tell 5e-specific content apart from framework-agnostic code at a glance. `combatTurn.js`, `npcBuilder.js`, `validateCharacter.js`, and `pointBuy.js` stay directly under `rules/` (not 5e-tool-specific, or not currently wired into either tool). Some of these files build their data path from `__dirname` instead of a plain `require()` specifier (`classFeatures.js`, `subclasses.js`, `featureCatalog.js`, `spellLists.js`) — if any of them ever move again, those lines need manual updating, an IDE's automated import-rewrite won't catch them.

Inside `engine/`:

- `rules/5e/classFeatures.js` / `rules/5e/subclasses.js` — load `data/5e/classes/*.json` / `data/5e/subclasses/*.json`. **Features are referenced by ID, not name** (`features_by_level: {"3": ["some-feature-id"]}`) — resolved back to display names transparently at load time via `rules/5e/featureCatalog.js` (a local id→name index, `data/5e/feature-catalog.json`, merged from the SRD feature cache + `published_features.json`). This exists because multiple real 5e features share a name across different classes/subclasses (e.g. "Spellcasting"), and a name-only lookup can silently resolve to the wrong one. When adding a new class/subclass feature, give it a real id — reuse an SRD index or a `published_features.json` id if one already exists for that exact feature, otherwise write a new `published_features.json` entry and reference its id.
- `rules/5e/spellcasting.js` — slot/cantrip/known-spell tables per class, resolved via `resolveSpellcasting(className, subclassName)` so a subclass can grant spellcasting a base class doesn't have (Eldritch Knight, Arcane Trickster — "third caster," PHB's level÷3 multiclass rate) without every caller needing to know which case it's in.
- `rules/5e/levelUp.js` (`describeLevelUp`) computes what a single class's level-up _should_ look like in isolation (pure, no character shape knowledge). `rules/5e/diffLevelUp.js` (`diffLevelUp`) is the adapter — the one other place besides `validateCharacter.js` that knows the `characters.json` shape, diffs `describeLevelUp`'s output against a real character, and returns `{ patch, pendingChoices, warnings }` for a caller to apply. `diffLevelUp` only levels up a class the character already has — picking up a brand-new class via multiclassing isn't supported yet (see TODO.md).
