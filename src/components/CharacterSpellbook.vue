  ...
  <div class="spellbook" v-if="character">
    ...
    <div class="sb-topbar">
      ...
      <div v-if="slotEntries.length" class="sb-slots-bar">
        <div v-for="[key, slot] in slotEntries" :key="key" class="sb-slot-level">
          <span class="sb-slot-label">{{ levelLabel(key) }}</span>
          <span
            v-for="i in slot.max" :key="i"
            class="sb-slot-pip"
            :class="{ spent: i <= slot.max - slotCurrent(slot) }"
            :title="i <= slot.max - slotCurrent(slot) ? 'Spent — click to recover' : 'Available — click to spend'"
      ...
      <div v-if="character.pact_magic" class="sb-slots-bar">
        <div class="sb-slot-level">
          <span class="sb-slot-label">Pact L{{ character.pact_magic.level ?? '?' }}</span>
          <span
            v-for="i in character.pact_magic.max" :key="i"
            class="sb-slot-pip sb-slot-pip--pact"
            :class="{ spent: i <= character.pact_magic.max - slotCurrent(character.pact_magic) }"
            :title="i <= character.pact_magic.max - slotCurrent(character.pact_magic) ? 'Spent — click to recover' : 'Available — click to spend'"
      ...
      <div v-if="preparationInfo" class="sb-prep-counter">
        ...
        <span class="sb-prep-fraction" :class="{ 'sb-prep-over': preparationInfo.over }">
          {{ preparationInfo.prepared }}<span class="sb-prep-max"> / {{ preparationInfo.max }}</span>
        </span>
        <div class="sb-prep-bar">
          <div class="sb-prep-bar-fill"
            :class="{ 'sb-prep-bar-over': preparationInfo.over }"
            :style="{ width: Math.min(100, preparationInfo.prepared / preparationInfo.max * 100) + '%' }">
          </div>
        </div>
        <span class="sb-prep-hint">{{ preparationInfo.ability.toUpperCase() }} mod + level · domain spells free · click a dot to toggle</span>
      </div>
    </div>

    <!-- ── Spell groups (spells in character record) ── -->
    <div v-for="group in spellGroups" :key="group.level" class="sb-group">
      <div class="sb-group-header">
        <span class="sb-group-label">{{ group.label }}</span>
        <span v-if="group.slotInfo" class="sb-slot-badge">{{ group.slotInfo }}</span>
        <span v-if="group.unpreparedCount && preparationInfo && !characterUsesFullClassList" class="sb-group-sub">
          {{ group.preparedCount }} prepared · {{ group.unpreparedCount }} unprepared
        </span>
      </div>
      ...
      <div class="sb-spell-grid">
        ...
        >
          ...
          ></button>
          <span class="sb-name" @click="openSpell(spell)" :title="spell.name">{{ spell.name }}</span>
          <span v-if="spell.domain" class="sb-badge sb-badge--domain" title="Domain — always prepared">D</span>
          <span v-else-if="spell.artillerist" class="sb-badge sb-badge--artillerist" title="Artillerist spell">A</span>
          <span v-else-if="spell.homebrew" class="sb-badge sb-badge--homebrew" title="Homebrew">H</span>
          <span v-else-if="spell.featureGranted" class="sb-badge sb-badge--feat" :title="spell._source">F</span>
          <span v-if="spellMeta[spell.name] && spellMeta[spell.name].concentration" class="sb-badge sb-badge--conc" title="Concentration">C</span>
          <span v-if="spellMeta[spell.name]" class="sb-badge sb-badge--action">{{ actionLabel(spellMeta[spell.name].actionType) }}</span>
        </div>
      </div>

      <!-- Unprepared spells (Wizard/Artificer spellbook only — not shown for full-class-list classes) -->
      <template v-if="preparationInfo && !characterUsesFullClassList && group.unprepared.length">
        ...
        <div class="sb-spell-grid sb-spell-grid--dim">
          ...
          >
            <button
              ...
              @click.stop="togglePrepared(spell)"
            ></button>
            <span class="sb-name sb-name--unprepared" @click="openSpell(spell)" :title="spell.name">{{ spell.name }}</span>
            <span v-if="spellMeta[spell.name] && spellMeta[spell.name].concentration" class="sb-badge sb-badge--conc" title="Concentration">C</span>
            <span v-if="spellMeta[spell.name]" class="sb-badge sb-badge--action">{{ actionLabel(spellMeta[spell.name].actionType) }}</span>
          </div>
        </div>
    ...
    <div v-if="!spellGroups.length" class="sb-empty">No spells recorded for this character.</div>
    ...
    <div v-if="characterUsesFullClassList && classSpellList" class="sb-browse">
      <div class="sb-browse-header">
        ...
        <span class="sb-browse-count">{{ availableToPrepare.length }} available</span>
        <div class="sb-browse-controls">
          ...
          <select v-model.number="availableLevelFilter" class="sb-level-select">
            ...
            <option v-for="l in maxSpellLevel" :key="l" :value="l">Level {{ l }}</option>
          </select>
        </div>
      </div>

      <div v-if="preparationInfo && preparationInfo.over" class="sb-browse-warning">
        Preparation limit reached ({{ preparationInfo.prepared }} / {{ preparationInfo.max }}) — unprepare a spell first
      </div>

      <div v-if="availableToPrepare.length" class="sb-spell-grid sb-spell-grid--dim">
        <div
        ...
        >
          <button
            ...
            :title="preparationInfo && preparationInfo.over ? 'Preparation limit reached' : 'Click to prepare'"
            @click.stop="prepareFromClassList(spell)"
          ></button>
          <span class="sb-name sb-name--available" @click="openSpell(spell)" :title="spell.name">{{ spell.name }}</span>
          <span class="sb-badge sb-badge--level">{{ spell.level === 0 ? 'C' : spell.level }}</span>
        </div>
      </div>
      <div v-else-if="availableSearch" class="sb-browse-empty">No spells match "{{ availableSearch }}"</div>
      <div v-else class="sb-browse-empty">All {{ character.class }} spells are in your prepared list.</div>
    </div>
...
import { getCharacterSpells, getClassSpellList, usesFullClassList } from '@/utils/spellUtils.js'
...
const PREPARATION_CLASSES = ['cleric', 'druid', 'wizard', 'artificer', 'paladin', 'ranger']
...
export default {
  ...
  computed: {
    ...
    availableToPrepare() {
      ...
      return this.classSpellList.filter((s) => {
        ...
        if (this.availableLevelFilter > 0 && s.level !== this.availableLevelFilter) return false
    ...
    slotEntries() {
      ...
      return Object.entries(slots).sort((a, b) => {
        return Number(a[0].replace('level_', '')) - Number(b[0].replace('level_', ''))
      })
    },
    ...
    preparationInfo() {
      ...
      const effectiveLevel = isHalfCaster ? Math.max(1, Math.floor(level / 2)) : level
      ...
      const prepared = this.allSpells.filter(
        (s) => s.level > 0 && !s.domain && !s.artillerist && !s.featureGranted && !s.homebrew && this.isReady(s)
      ).length
    ...
    spellGroups() {
      ...
      return Object.keys(byLevel)
        ...
        .map((lvl) => {
          ...
          return {
            level: lvl,
            label: lvl === -1 ? 'Feature Spells' : lvl === 0 ? 'Cantrips' : `Level ${lvl}`,
            prepared,
            unprepared,
  ...
  methods: {
    ...
    isReady(spell) {
      ...
      if (spell.domain || spell.artillerist || spell.featureGranted || spell.homebrew) return true
    ...
    canToggle(spell) {
      ...
      if (spell.domain || spell.artillerist || spell.featureGranted || spell.homebrew) return false
    ...
    prepTitle(spell) {
      ...
      if (spell.featureGranted || spell.homebrew) return `Granted by ${spell._source} — always available`
      if (!this.preparationInfo) return 'Known spell — always available'
      return spell.prepared ? 'Prepared — click to unprepare' : 'Not prepared — click to prepare'
    },
    ...
    togglePrepared(spell) {
      ...
      if (this.characterUsesFullClassList && spell.prepared) {
        ...
        updated = (this.character.spells ?? []).filter((s) => s.name !== spell.name)
      } else {
    ...
    prepareFromClassList(spell) {
      ...
      const existing = (this.character.spells ?? []).find((s) => s.name === spell.name)
      let updated
      if (existing) {
        ...
        updated = [...(this.character.spells ?? []), { name: spell.name, level: spell.level, prepared: true }]
      }
      this.$store.commit('UPDATE_TABLE_ITEM', {
        table: 'characters',
        updatedItem: { ...this.character, spells: updated },
      ...
      this.$store.commit('UPDATE_TABLE_ITEM', {
        ...
        updatedItem: {
          ...
          spell_slots: {
            ...
            [key]: { ...slot, current: Math.min(slot.max, Math.max(0, newCur)) },
          },
        },
      })
    },
    ...
    async loadMeta() {
      ...
      await Promise.all(
        this.allSpells.map(async (s) => {
          ...
          meta[s.name] = { concentration: !!data.concentration, actionType, school: data.school ?? '' }
          if (s.level === null && data.level != null) levels[s.name] = data.level
        })
      )
      this.spellMeta      = meta
    ...
    async openSpell(spell) {
      ...
      const levelStr = lvl === 0 ? 'Cantrip' : lvl != null ? `Level ${lvl}` : 'Spell'
...
.sb-slot-pip:hover         { background: rgba(200, 169, 110, 0.6); }
.sb-slot-pip.spent         { background: transparent; border-color: var(--color-border); }
.sb-slot-pip--pact         { border-color: #8866dd; background: rgba(136, 102, 221, 0.3); }
.sb-slot-pip--pact:hover   { background: rgba(136, 102, 221, 0.6); }
.sb-slot-pip--pact.spent   { background: transparent; border-color: var(--color-border); }
...
.sb-prep-fraction.sb-prep-over { color: #c0442a; }
.sb-prep-max { font-weight: normal; color: var(--color-text-low); }
...
.sb-prep-bar-fill.sb-prep-bar-over { background: #c0442a; }
...
.sb-spell-grid--dim:hover { opacity: 0.8; }
...
.sb-spell-entry:hover { background: rgba(255,255,255,0.05); }
...
.sb-dot:disabled { cursor: not-allowed; opacity: 0.35; }
.sb-dot--ready  { background: #5a9e5a; border-color: #5a9e5a; }
.sb-dot--fixed  { cursor: default; }
.sb-dot--ready.sb-dot--fixed { background: var(--color-accent); border-color: var(--color-accent); }
...
.sb-name:hover         { color: var(--color-accent); }
.sb-name--unprepared   { color: var(--color-text-muted); }
.sb-name--available    { color: var(--color-text-muted); }
...
.sb-badge--domain      { border-color: var(--color-accent); color: var(--color-accent); }
.sb-badge--artillerist { border-color: #4488cc; color: #4488cc; }
.sb-badge--homebrew    { border-color: #cc7744; color: #cc7744; }
.sb-badge--feat        { border-color: #88aa44; color: #88aa44; }
.sb-badge--conc        { border-color: #8866dd; color: #8866dd; }
.sb-badge--action      { border-color: var(--color-border); color: var(--color-text-low); }
.sb-badge--level       { border-color: var(--color-border); color: var(--color-text-low); min-width: 12px; text-align: center; }
...
.sb-level-select:focus { border-color: var(--color-accent); }
