...
- [ ] **Label/move the engine's 5e-specific rules files, so a future ruleset (the planned Godot port, or "modernizing" the game itself) is easy to tell apart from generic engine plumbing — requested 2026-09-17.** Project owner's call: header-comment-only labeling wasn't quite what they wanted after seeing the actual file list — they'd rather move the relevant files into a `5e/` subfolder themselves via VS Code's automated move-and-update-imports, and asked for the exact file list first. That list, worked out by tracing the real dependency closure of everything `NewCharacterTool.vue`/`LevelUpTool.vue` actually call through `server.js`'s `/api/engine/*` routes (not guessed):

  **`engine/rules/*.js`, 19 files** — `abilities.js`, `asiFeat.js`, `backgrounds.js`, `classFeatures.js`, `diffLevelUp.js`, `featPrerequisites.js`, `featureCatalog.js`, `fightingStyles.js`, `grants.js`, `invocations.js`, `languages.js`, `levelUp.js`, `multiclass.js`, `progression.js`, `skills.js`, `species.js`, `spellLists.js`, `spellcasting.js`, `subclasses.js`.

  **`engine/data/*`** — `backgrounds.json`, `favored-enemies.json`, `feats.json`, `fighting-styles.json`, `invocations.json`, `languages.json`, `leveling.json`, `multiclass-prerequisites.json`, `multiclass-proficiencies.json`, `natural-explorer-terrains.json`, `pact-boons.json`, `skills.json`, `species.json`, `spellcasting-tables.json`, `feature-catalog.json`, plus the whole `classes/` and `subclasses/` folders.

  **Deliberately excluded**: `combatTurn.js` (Combat Tracker, unrelated), `npcBuilder.js` (NPC Builder tool, unrelated), `validateCharacter.js` (not wired into either tool's UI today — engine-test-only). `pointBuy.js` is a judgment call: New Character doesn't call it over the API, but its own code comment says the client-side ability-score UI duplicates that file's table for instant feedback — project owner can decide whether that counts.

  **Real gotcha if this move actually happens** (whether done by hand or by Claude later): `classFeatures.js`, `subclasses.js`, and `featureCatalog.js` don't load their JSON via a normal `require('../data/...')` — they build the path at runtime with `path.join(__dirname, '..', 'data', 'classes')` etc. (`classFeatures.js:5`, `subclasses.js:5`, `featureCatalog.js:16`). An IDE's "update imports on move" only rewrites real `require()`/`import` specifiers, not a string built from `__dirname` — so if these three files end up a different number of directories away from `engine/data/` after the move, those 3 lines will silently point at the wrong path (a runtime file-not-found, not a build error) until fixed by hand to match the new relative distance.

- [ ] **Full class/subclass feature audit — discovery pass done 2026-09-16, real fixes not started.** Confirmed pattern: Fighting Style (fixed 2026-09-09), Favored Enemy + Natural Explorer (fixed 2026-09-16), and Expertise (fixed 2026-09-17) were all the same root bug — a `features_by_level` entry resolves to a generic catalog name with zero mechanism turning it into a real `pendingChoice`, so New Character/Level Up just list a bare, unpickable feature forever. Ran a systematic scan of every base class (`engine/data/classes/*.json`) and every subclass (`engine/data/subclasses/*.json`) against `diffLevelUp.js`'s actual `pendingChoices.push({type: ...})` call sites (the only reliable ground truth — currently: `asiOrFeat`, `fightingStyleChoice`, `expertiseChoice`, `favoredEnemyChoice`, `naturalExplorerChoice`, `invocationChoice`, `pactBoonChoice`, `bonusSpellChoice`, `subclassChoice`, `multiclassSkillChoice` — anything else is unwired by definition).
  ...
  **Confirmed real gaps, base classes** (verified against each feature's actual stored description text, not just guessed from a keyword match):
  ...
  - **Sorcerer's Metamagic** (L3: pick 2 options; L10/L17: +1 each) — zero wiring anywhere (`grep -n metamagic engine/rules/diffLevelUp.js` returns nothing). High value — Metamagic is Sorcerer's core identity and every Sorcerer hits L3.
...
- [ ] **`fighting-styles.json` only has the 6 core PHB styles — missing Tasha's Cauldron of Everything's 5 additions, found 2026-09-16.** Current catalog (Fighter 6/Paladin 4/Ranger 4, all core PHB) doesn't include Blind Fighting, Interception, Superior Technique, Thrown Weapon Fighting, or Unarmed Fighting. Per a web check, Blind Fighting is available to all three classes but the other four aren't necessarily universal — exact per-class availability needs verifying against `dnd5e.wikidot.com` (per this project's usual RAW-verification standard) before adding, not assumed from a blended search summary. **Scope question resolved 2026-09-17: project owner's call is to use every official WotC 2014-ruleset sourcebook during R&D (PHB, XGE, TCE, SCAG, VRGtR, etc. all in bounds) — this app will get pared down to the campaign's actual chosen flavor later, once building the real game, not now. Still 2014 rules only, never 2024/One D&D content — that line hasn't moved.** Not started.

- [ ] **Gloom Stalker Ranger's tiered bonus spells (Gloom Stalker Magic) have zero engine wiring — found 2026-09-17.** Unlike species-level tiered spells (Drow Magic, Infernal Legacy — real `tiered` arrays in `species.json`, fully handled by `diffLevelUp.js`), `ranger-gloom-stalker.json`'s `features_by_level` only lists `pub_gloom-stalker-magic` once, at level 3 — its stored description mentions all 5 real tiers (3rd Disguise Self, 5th Rope Trick, 9th Fear, 13th Greater Invisibility, 17th Seeming, verified against the feature's own stored text and confirmed as real 2014 XGE content, not a 2024/UA substitution) but nothing in `diffLevelUp.js` actually grants the later 4 spells when a Gloom Stalker Ranger crosses those levels — they'd need to be added to the character by hand today. Needs the same `tiered` structure + wiring pattern already proven for species traits, generalized to subclass features (this is presumably not unique to Gloom Stalker — worth checking other subclasses with "gain an additional spell at Nth level" text, e.g. Fey Wanderer, while building this).

- [ ] **Subrace mechanics — mostly resolved 2026-09-16, one piece still open.** Re-investigated this whole item and found its original premise partly stale: `traitsFor(race, subrace)` already merges subrace traits into `raceTraits` in `diffLevelUp.js`, and that function already drives both the flat HP-per-level bonus (Dwarven Toughness) and tiered bonus spells (Drow Magic, Infernal Legacy) — fully built and covered by `engine/test/speciesLevelUpMechanics.test.js` since 2026-09-10, not missing engine plumbing as this item originally claimed. `NewCharacterTool.vue`'s creation-time wiring (`resolvedSpeciesGrants`, `speciesTraitRecords`, `speciesLanguageChoiceTotal`) is equally complete — weapon/armor/tool/skill proficiency grants, resistances, saving-throw advantages, fixed/choice spells, and the subrace's own extra-language choice (High Elf) all resolve correctly for a character built through the tool today. **Confirmed via Torrin**, who already has real `species_traits` (including Dwarven Toughness) — his 2026-09-12 Level Up tool rebuild picked all of this up automatically with zero extra work, which is the best evidence the mechanism holds up end-to-end.
  ...
  **Still open**: Kessara (High Elf) and Jaygar (Rock Gnome) both predate the 2026-09-07 mechanism too and are still missing their subrace's `species_traits`/proficiency/resistance grants (Kessara: Elf Weapon Training, the Cantrip trait's `spells_granted` tie-in for her existing Minor Illusion pick, Extra Language; Jaygar: Gnome Cunning's saving-throw advantage, Artificer's Lore, Tinker's tool proficiency). Left alone this pass to avoid touching two more already-played, level-9 sheets under time pressure in the same session as the Pirra/Tackett fix — worth a dedicated pass, not a rushed add-on.
